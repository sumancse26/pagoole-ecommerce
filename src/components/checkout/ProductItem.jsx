...
const ProductItem = () => {
    ...
    useEffect(() => {
        const getProduct = () => {
            ...
            if (!itemStr) {
                setCheckoutData([]); // Set empty array if no data
                return;
            }
            const item = JSON.parse(itemStr);
            console.log('get item', item.checkout_info || []);
            setCheckoutData(item.checkout_info || []);
        };

        // If you want a 1s delay, you can keep setTimeout here:
                            ...
                            <div className="relative bg-gradient-to-tr from-green-50 to-green-100 rounded-xl shadow-lg overflow-hidden border border-green-200">
                                ...
                                <div className="px-6 py-4 space-y-3">
                                    {vendor?.items?.length > 0 ? (
                                        ...
                                        <p className="text-gray-400 py-4 text-center">No products for this vendor.</p>
                                    )}
                                </div>
                                ...
                                <div className="flex flex-col sm:flex-row justify-between items-center gap-3 px-6 py-4 border-t border-green-200 bg-gradient-to-r from-green-100/40 to-green-50/40">
                                    ...
                                    <button className="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-lg font-semibold shadow transition">
                                        Download Invoice
                                    </button>
                                </div>
                            </div>
